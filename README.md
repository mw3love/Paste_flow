...
- **순차 경로 붙여넣기** — 큐의 이미지를 임시 PNG로 저장한 절대경로 텍스트로 순서대로 붙여넣기. `Ctrl+Shift+P`=하나씩, `Ctrl+Shift+[`=전체 (Claude CLI 워크플로용)
- **화면 OCR** — `Ctrl+Shift+S`로 화면 영역 선택 → 텍스트 추출 (Windows WinRT 무료 / AI API — Gemini·Claude·GPT 등 게이트웨이 모델 선택 가능)
- **Gemini에게 질문** — 이미지 우클릭 → 이미지를 첨부한 질문창을 띄워 Google AI 모드(브라우저)로 보낸다
- **시스템 트레이** — 백그라운드 상주, 트레이 아이콘으로 패널 열기
- **단축키 커스터마이징** — 설정창에서 기능별로 단축키 변경 가능
...
| `Ctrl+Shift+P` | 큐에서 다음 항목을 붙여넣되 이미지는 임시 PNG 경로 텍스트로 (기본값, 설정에서 변경 가능) |
| `Ctrl+Shift+[` | 위 경로 붙여넣기를 큐 끝까지 자동으로 (기본값, 설정에서 변경 가능) |
