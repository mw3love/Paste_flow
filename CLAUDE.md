...
- **`panel.py`** — 고정 섹션 + 히스토리 패널 (검색 기능 없음 — 의도적으로 제거).
  ...
  - **우클릭 컨텍스트 메뉴**: "큐에 추가"(`queue_select_requested`) / 고정·해제 / 복사 / 수정(텍스트만) / 텍스트 추출(OCR)(이미지만 — `ocr_item_requested` emit) / 삭제 / 미리보기(이미지→`preview_image_requested` emit, 텍스트→`preview_text_requested` emit). 둘 다 main에서 받아 `ImagePreviewPopup.open_new(item, ...)` / `TextPreviewPopup.open_new(item, ...)` 호출 (동일 패턴 — 둘 다 `ClipboardItem`을 인자로 받음).
