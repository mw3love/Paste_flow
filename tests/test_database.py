...
class TestAiHistory:
    ...
    def test_save_returns_id(self, db):
        """저장 시 새 row id 반환"""
        hid = db.save_ai_conversation("질문", '[{"role": "user", "content": "질문"}]',
                                      "gemini-2.5-flash", "official")
        assert hid is not None
        assert hid > 0

    def test_get_conversation_round_trip(self, db):
        """저장한 conversation·model·backend·title이 그대로 조회된다"""
        conv = '[{"role": "user", "content": "안녕"}, {"role": "assistant", "content": "네"}]'
        hid = db.save_ai_conversation("안녕", conv, "gemini-2.5-flash", "official")
        row = db.get_ai_conversation(hid)
        assert row["title"] == "안녕"
        assert row["conversation"] == conv
        assert row["model"] == "gemini-2.5-flash"
        assert row["backend"] == "official"

    def test_get_conversation_missing_returns_none(self, db):
        """없는 id 조회 시 None"""
        assert db.get_ai_conversation(999) is None

    def test_update_conversation(self, db):
        """후속 질문 도착 시 conversation 갱신 — title/model은 불변"""
        hid = db.save_ai_conversation("질문", "[]", "gemini-2.5-flash", "official")
        db.update_ai_conversation(hid, '[{"role": "user", "content": "후속"}]')
        row = db.get_ai_conversation(hid)
        assert row["conversation"] == '[{"role": "user", "content": "후속"}]'
        assert row["title"] == "질문"

    def test_list_orders_by_recently_updated(self, db):
        """목록은 최근 갱신순(updated_at DESC)"""
        first = db.save_ai_conversation("먼저", "[]", "m", "official")
        second = db.save_ai_conversation("나중", "[]", "m", "official")
        db.update_ai_conversation(first, '[{"role": "user", "content": "다시"}]')
        records = db.get_ai_history_list()
        assert records[0]["id"] == first  # 방금 갱신된 쪽이 맨 위

    def test_list_excludes_conversation_body(self, db):
        """목록 조회는 대화 본문을 담지 않는다(표시용 요약만)"""
        db.save_ai_conversation("질문", '[{"role": "user", "content": "본문"}]', "m", "official")
        records = db.get_ai_history_list()
        assert "conversation" not in records[0]

    def test_delete_conversation(self, db):
        """삭제 후 조회하면 None, 목록에서도 빠진다"""
        hid = db.save_ai_conversation("질문", "[]", "m", "official")
        db.delete_ai_conversation(hid)
        assert db.get_ai_conversation(hid) is None
        assert all(r["id"] != hid for r in db.get_ai_history_list())
