...
구글 드라이브·PasteFlow 자체 답변·사용자가 추가한 웹사이트) 중 하나로 보낸다. 목적지
목록은 설정 DB에 JSON으로 저장돼 사용자가 직접 추가·삭제·순서 변경할 수 있다(설정창
"AI 팔레트 타겟" 그룹, `ui/settings_dialog.py`). 새 외부 의존성 없음 — stdlib
`json`/`urllib.parse`만 사용(프로젝트 "새 외부 의존성 0" 원칙).

각 타겟은 `{"label": str, "keyword": str, "kind": str, "url": str}` — `kind`가 셋의
특수 경로(`KIND_GOOGLE_AI`/`KIND_DRIVE`/`KIND_API`)면 실제 실행은 main.py의 기존 배관
(web_open.py의 구글 AI·드라이브 검색, AI 워커)이 그대로 담당하고 `url`은 쓰이지 않는다.
`KIND_URL`만 이 모듈의 `build_url()`로 `{q}` 자리에 질의를 채워 넣는다.
"""
...
# 첫 실행(저장된 값이 없을 때) 기본 타겟 — 순서가 곧 팔레트 번호(Alt+1~6).
# Gemini 웹앱은 URL의 q 파라미터를 무시해(2026-07-14 실측, web_open.py 참고) 목록에서
# 뺐다 — 같은 역할을 구글 AI 모드가 URL만으로 더 안정적으로 해낸다.
