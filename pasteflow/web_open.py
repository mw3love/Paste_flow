...
--------------------------------------
"오늘 SK하이닉스 주가"를 API 검색 경로에 세 번 물었더니 현재가를 **한 번도 못 맞혔다**
(205만·208만·175만 — 실제 191.3만). 전일 종가 같은 '정적 사실'은 다 맞히는데 현재가만
틀린다.
...
받아적기(API)에서 지던 싸움을 그만두고 전광판 앞에 데려다 놓는 것이다. v1.6x에서 AI 질의
기능 자체를 제거해, 이제 이 모듈은 AI 팔레트(Alt+`)의 Google AI 타겟 하나만 연다.

한계 — 답은 크롬에 뜨고 끝난다
------------------------------
프로그램이 그 답 텍스트를 읽지 못하므로 PasteFlow의 답변창 렌더 등은 이 경로에 적용되지
않는다. 되가져오려면 크롬 확장이 필요하다(별개 작업).

URL 프리필 실측
---------------
- 구글 AI 모드: `?q=…&udm=50` → **질문이 자동 전송되고 답이 렌더된다**(로그인 불필요).
- 이미지 첨부가 있으면 URL에 실을 수 없어 홈 화면(`google_ai_home_url`)을 열고 클립보드
  붙여넣기로 주입한다(구글 AI 모드는 로드 직후 입력칸에 자동 포커스가 간다).
"""
